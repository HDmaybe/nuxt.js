document.addEventListener("DOMContentLoaded", function(event) { 
    function init() {
        ...
        close_btn2.setAttribute('class', 'close-btn material-symbols-outlined');
        ...
        textarea.onkeyup = function k(event){
    ...
    function second(){
        ...
        chatbotToggler.addEventListener("click",()=> {
           if (tokyo.classList.contains('show-chatbot')) {
            tokyo.classList.remove('show-chatbot')
           } else {
            tokyo.classList.add('show-chatbot')
           }
        }); 
        let userMessage;
        ...
        const handleChat = () => {
            ...
            xhrPost.open("POST", "http://restory.intellisys.co.kr:9998/chat/post");
            xhrPost.setRequestHeader("Content-Type", "application/json");
            xhrPost.onreadystatechange = function () {
                if (xhrPost.readyState === XMLHttpRequest.DONE) {
                    if (xhrPost.status === 200) {
                        const xhrGet = new XMLHttpRequest();
                        xhrGet.open("GET", "http://restory.intellisys.co.kr:9998/chat/get?question=" + encodeURIComponent(userMessage));
                        xhrGet.onreadystatechange = function () {
                            if (xhrGet.readyState === XMLHttpRequest.DONE) {
                                if (xhrGet.status === 200) {
                                    ...
                                    console.error("Error:", xhrGet.statusText);
                                }
                            }
                        };
                        xhrGet.send();
            ...
            xhrPost.send(JSON.stringify({ question: userMessage }));
        ...
        chatbotCloseBtn.addEventListener("click", () => {tokyo.classList.remove('show-chatbot')});
